                    ...
                    <form action="{{ route('admin.team.update', $team->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                <option value="leadership" {{ old('type', $team->type) == 'leadership' ? 'selected' : '' }}>Leadership</option>
                                <option value="member" {{ old('type', $team->type) == 'member' ? 'selected' : '' }}>Member</option>
                            </select>
                        ...
                        <div>
                            ...
                            @if($team->gallery)
                                <div class="mb-2">
                                    <img src="{{ $team->gallery->image_path }}" class="h-20 w-20 object-cover rounded-full">
                                </div>
                            @endif
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Update Member') }}</x-primary-button>
